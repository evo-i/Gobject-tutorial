...
Пример в этом разделе - объект TNumStr.
TNumStr является потомком объекта TStr.
TNumStr хранит строку и тип строки, который может быть `t_int`, `t_double` или `t_none`.

- t\_int: строка представляет целое число
- t\_double: строка представляет число с плавающей точкой (double)
- t\_none: строка не представляет ни одно из двух чисел выше.

Строка типа t\_int или t\_double называется числовой строкой.
Это не общепринятая терминология, она используется только в этом руководстве.
Короче говоря, числовая строка - это строка, которая представляет число.
Например, "0", "-100" и "123.456" являются числовыми строками.
Это строки, которые представляют числа.

- "0" - это строка.
Это массив символов, элементами которого являются '0' и '\0'.
Она представляет 0, то есть целое число ноль.
- "-100" - это строка, состоящая из '-', '1', '0', '0' и '\0'.
Она представляет целое число -100.
- "123.456" состоит из '1', '2', '3', '.', '4', '5', '6' и '\0'.
Она представляет действительное число (тип double) 123.456.

Числовая строка - это такая специфическая строка.
...
Нужно быть осторожными, так как "0" и "0.0" различаются.
Потому что их типы различны.
Тип "0" - целое число, а тип "0.0" - double.
Аналогично, "1" - это целое число, а "1." - это double.

".5" и "0.5" - одно и то же.
Оба являются double и их значения равны 0.5.

Проверка числовой строки - это вид лексического анализа.
Для лексического анализа часто используется диаграмма состояний и матрица состояний.

Числовая строка - это последовательность символов, которая удовлетворяет:

1. '+' или '-' может быть первым символом. Может быть опущен.
...
- 6 - ошибка. Строка не представляет число.
...
Эта диаграмма состояний не поддерживает стиль double "1.23e5" (десятичная с плавающей точкой).
Если бы это поддерживалось, диаграмма состояний была бы более сложной.
(Однако, это будет хорошей практикой для ваших навыков программирования.)

## Заголовочный файл

Заголовочный файл TNumStr - [`tnumstr.h`](../src/tstr/tnumstr.h).
Он находится в директории `src/tstr`.
...
- 11-15: Эти три enum данные определяют тип строки TNumStr.
  ...
  - `t_int`: Строка представляет целое число
  - `t_double`: Строка представляет действительное число, которое является типом double в языке C.
...
C файл TNumStr - [`tnumstr.c`](../src/tstr/tnumstr.c).
Он находится в директории `src/tstr`.
...
- 9-12: Структура TNumStr имеет родительский "TStr" и члены типа int "type".
Таким образом, экземпляр TNumStr хранит строку, которая размещается в приватной области родителя, и тип.
- 14: Макрос `G_DEFINE_TYPE`.
- 16- 54: Функция `t_num_str_string_type` проверяет данную строку и возвращает `t_int`, `t_double` или `t_none`.
Если строка NULL или не является числовой строкой, будет возвращено `t_none`.
Алгоритм проверки объясняется в первом подразделе "Проверка числовой строки".
- 60-64: Функция `t_num_str_real_set_string` устанавливает строку TNumStr и её тип.
...
Когда вы хотите установить строку в экземпляр TNumStr, вы можете использовать функцию `t_str_set_string`.
...
// => на экране появится t_double
```

TNumStr добавляет несколько публичных функций.
...
В директории `src/tstr` находятся файлы `main.c`, `test1.c` и `test2.c`.
...
Сначала измените текущую директорию на `src/tstr`.
...
Последняя часть `main.c` - это преобразование между TNumStr и TNumber.
Между ними есть некоторые различия по следующим двум причинам.

- числа с плавающей точкой округляются.
Это не точное значение, когда значение имеет много цифр.
...
Сложно сравнивать два экземпляра TNumStr.
Лучший способ - сравнить экземпляры TNumber, преобразованные из TNumStr.
