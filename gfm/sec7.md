...
Создание не абстрактного производного типа встречается чаще, чем создание абстрактного типа.
Этот раздел охватывает, как создавать объекты не абстрактного производного типа.
Пример производного типа - это объект для строки.
Это TStr.
...
Он похож на TInt или TDouble, но строка более сложна, чем int и double.
Когда вы создаете программу TStr, вам нужно быть осторожным с управлением памятью, что не требуется для TInt и TDouble.
...
Следовательно, указатель не является строкой.
...
Поскольку строка является массивом символов, для создания новой строки необходимо выделение памяти.
...
- Если аргумент равен NULL, то память не выделяется и возвращается NULL.
...
Строки, заключенные в двойные кавычки, такие как "Hello.", являются строковыми литералами.
Они представляют собой массив символов, но содержимое массива не разрешено изменять.
И их нельзя освобождать.
Если вы запишете символ в строковый литерал или освободите строковый литерал, результат будет неопределенным.
...
Если массив инициализирован строковым литералом, массив может быть изменен.
...
Он может находиться в области кода программы или в какой-то другой области, недоступной для записи.
Это зависит от реализации вашего компилятора.
Поэтому вторая строка пытается записать символ 'a' в память только для чтения, и результат будет неопределенным, например, произойдет ошибка сегментации.
В любом случае, не пишите программу таким образом.
...
Область стека исчезнет, когда функция вернется к вызывающей стороне.
- Статическая область.
Если класс массива - `static`, то массив размещается в статической области.
Он сохраняет свое значение и остается в течение всего времени работы программы.
...
Поэтому вы не можете изменить `t`, потому что `t` указывает на строковый литерал, который доступен только для чтения.

Второй способ - создание памяти и копирование строки в память.
...
Функция `g_strdup` выделяет память и инициализирует ее "Hello", затем возвращает указатель на память.
Функция `g_strdup` почти такая же, как функция `string_dup` ниже.
...
Если используется `g_strdup`, два указателя `s` и `t` указывают на разные участки памяти.
...
Квалификатор `const` делает так, что переменная не будет изменять свое значение.
Его также можно применить к массиву.
...
Таким же образом возвращаемое значение (указатель на массив или строку) функции может быть квалифицировано с помощью `const`.
...
GValue, на который указывает `value`, принадлежит вызывающей стороне, и функция не изменяет и не освобождает его.
...
TStr является потомком GObject и хранит строку.
...
Память для массива выделяется из области кучи.
TStr владеет памятью и отвечает за ее освобождение, когда она становится ненужной.
...
Поэтому и TStr, и TNumStr имеют член `set_string` в своих классах, но они указывают на разные функции.
- 14-15: Публичная функция `t_str_concat` соединяет две строки экземпляров TStr и возвращает новый экземпляр TStr.
- 18-22: Setter и getter.
...
Однако `enum` может использоваться для определения двух или более свойств, и это более распространено.
...
- 11-13: TStrPrivate - это C структура.
Это область приватных данных для TStr.
Если бы TStr был финальным типом, то не было бы потомков, и экземпляр TStr мог бы быть областью приватных данных.
Но TStr является производным, поэтому вы не можете хранить такие приватные данные в экземпляре TStr, который открыт для потомков.
Имя этой структуры - "<имя объекта\>Private" как `TStrPrivate`.
Структура должна быть определена перед `G_DEFINE_TYPE_WITH_PRIVATE`.
- 15: Макрос `G_DEFINE_TYPE_WITH_PRIVATE`.
Он похож на макрос `G_DEFINE_TYPE`, но добавляет область приватных данных для производного экземпляра.
...
Она указывает на родительский класс TStr.
  - Вызов функции, который добавляет приватные данные экземпляра к типу.
Это C структура, и ее имя - `TStrPrivate`.
  - Определение функции `t_str_get_type ()`.
Эта функция регистрирует тип при первом вызове.
  - Определение геттера приватного экземпляра `t_str_get_instance_private ()`.
- 17-28: Функция `t_str_set_property` устанавливает свойство "string" и используется функциями семейства `g_object_set`.
Она использует функцию `t_str_set_string` для установки приватных данных с копией строки из GValue.
Это важно, потому что `t_str_set_string` вызывает метод класса `set_string`, который будет переопределен дочерним классом.
Поэтому поведение функции `t_str_set_property` различается между TStr и TNumStr, который является потомком TStr.
...
- 30-40: Функция `t_str_get_property` получает свойство "string" и используется функциями семейства `g_object_get`.
Она просто передает `priv->string` функции `g_value_set_string`.
Переменная `priv` указывает на область приватных данных.
...
- 44-51 Функция `t_str_real_set_string` - это тело метода класса, на которое указывает член `set_string` в классе.
Сначала она получает указатель на приватную область с помощью функции `t_str_get_instance_private`.
Если экземпляр содержит строку, освобождает ее перед установкой новой строки.
Она копирует заданную строку и присваивает ее `priv->string`.
Дублирование важно.
Благодаря этому адрес строки скрыт от внешней стороны экземпляра.
...
На фазе dispose экземпляр освобождает экземпляры.
На фазе finalize экземпляр выполняет остальную часть всей финализации, такую как освобождение памяти.
Эта функция освобождает строку `priv->string`, если необходимо.
После этого она вызывает метод finalize родителя.
Это называется "chain up to its parent" (подъем к родителю) и будет объяснено позже в этом разделе.
- 63-68: Функция инициализации экземпляра `t_str_init` присваивает NULL переменной `priv->string`.
...
- 84-101: Setter и getter.
Метод setter `t_str_set_string` просто вызывает метод класса.
Поэтому ожидается, что он будет заменен в дочернем классе.
Он используется функцией установки свойства `t_str_set_property`.
Таким образом, поведение установки свойства изменится в дочернем классе.
Метод getter `t_str_get_string` просто дублирует строку `priv->string` и возвращает копию.
- 103-126: Публичная функция `t_str_concat` объединяет строки `self` и `other` и создает новый TStr.
- 129-137: Две функции `t_str_new_with_string` и `t_str_new` создают новые экземпляры TStr.

## Подъем к родителю (Chaining up to its parent)

Процесс "подъема к родителю" проиллюстрирован на диаграмме ниже.

![Процесс подъема в GObject и TStr](../image/chainup.png)

Существует два класса: GObjectClass и TStrClass.
Каждый класс имеет свои методы finalize (функции), на которые указывают указатели в структурах классов.
Метод finalize класса TStrClass финализирует свою собственную часть экземпляра TStr.
В конце функции он вызывает метод finalize родителя.
Это метод finalize класса GObjectClass.
Он вызывает свою собственную функцию finalize и финализирует приватные данные GObject.

Если GObjectClass имеет два или более дочерних класса, количество функций finalize может быть таким же, как количество потомков.
И они связаны способом "подъема к родителю".  

![Процесс подъема](../image/chainup3.png)
...
Большинство членов - это методы класса.
- Используйте `G_DEFINE_TYPE_WITH_PRIVATE` в C файле.
Вам нужно определить приватную область перед `G_DEFINE_TYPE_WITH_PRIVATE`.
Это C структура, и имя - "<имя объекта\>Private" как "TStrPrivate".
